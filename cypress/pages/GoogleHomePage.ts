...
class GoogleHomePage extends BasePage {
		...
		cookiesModalParts.forEach((part) => {
			...
			cy.contains(cookiesModal.header)
				...
				.then((scrollableParent) => {
					if (!scrollableParent) {
						return cy.log(
							'No scrollable parent found for the cookies modal header, moving on'
						)
					}
	...
	assertCookieMondalContentsAreNotVisible(language?: Language) {
		...
		const cookiesModalTranslations = getTranslation(
			language,
			(t) => t.cookiesModal
		)
		const cookiesModalContents: string[] = Object.entries(
			cookiesModalTranslations
		)
	...
	assertInitialLoadExpectedElements(language?: Language) {
		cy.getCountryFromCurrentIp().then((ipCountryName) => {
			...
			const countryNameTranslations: Record<
				string,
			...
			const homePageTranslations = getTranslation(
				userLanguage,
				(t) => t.homePage
			)
			...
			if (ipCountryName === 'Poland' && userLanguage === 'english') {
				visibleContents.push(
					`${homePageTranslations.googleOfferedIn} ${languageLabelMap.english}`
				)
			}
